...
Deliberately a leaf: it imports nothing from `loaddata`, `prepdata`, `pipeline`, `evaluate` or `interpret`. Keeping the
vocabulary they all share here is what lets `evaluate/` score results without importing `pipeline/`.
"""
