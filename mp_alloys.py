...
def get_stoich_array(X, PT):
    if type(X) == pd.DataFrame:
        formulas = X['composition'].copy()  # if user passes whole of Novamag
    else:
    ...
    for el in symbols:
        regex_list = formulas.str.extractall(
              pat=r"(?P<element>[A-Za-z]+)\s*(?P<digit>\d*)")
...
def get_Electronegw(PT, stoich_array):
    ...
    en_list = PT['electronegativity'].str.extract(
        pat=r"(?P<digit>\d*\.\d+)").astype(float)
    for i in range(len(stoich_array)):
        ...
        Electronegw.iloc[i] = np.dot(
            at_fraction, en_list.loc[compound.index[cols]])
    return Electronegw
